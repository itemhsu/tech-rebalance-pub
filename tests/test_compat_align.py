...
def _require_accounts():
    """public 鏡像無 accounts.json → skip（私有 repo 才有帳戶設定）。"""
    if not (ROOT / "accounts.json").exists():
        pytest.skip("無 accounts.json（public 鏡像）")
...
def test_dashboard_supports_emitted_schema_version():
    if not (ROOT / "mvp_dashboard.html").exists():
        pytest.skip("無 mvp_dashboard.html（public 鏡像）")
